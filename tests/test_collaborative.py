...
def collect_visit_statistics(solution):
    ...
    num_unique = len(vc)
    num_redundant = total_visits - num_unique
...
def run_multiagent_tests():
    """
    Teste le système multi-agent collaboratif sur toutes les instances
    ...
    for file_name in os.listdir(directory_path):
        ...
        # Créer le système multi-agent collaboratif
        ...
        # ============================================================
        # AGENTS ITÉRATIFS (exploit / explore)
        # ============================================================

        mas.add_agent(
            "HC", Hill_Climbing,
            run_params={'max_iterations': 1000, 'log_history': True},
            is_population_based=False, max_cycles=10, role="exploit"
        )
        ...
        mas.add_agent(
            ...
            is_population_based=False, max_cycles=10, role="explore"
        )

        # ============================================================
        # AGENTS À POPULATION (recombine)
        # ============================================================
        ...
        mas.add_agent(
            ...
            is_population_based=True, max_cycles=10, role="recombine"
        )
        ...
        mas_results = mas.run(verbose=False)
        ...
        # Le système collaboratif retourne les résultats par agent dans 'agents_results'
        all_agent_results = mas_results.get('agents_results', {})

        for agent_name, agent_data in all_agent_results.items():
            # Le format retourné par AsyncCollaborativeMultiAgentSystem est :
            # {'cycles', 'deposited', 'best_distance', 'best_solution'}
            dist = agent_data.get('best_distance', 0) or 0
            solution = agent_data.get('best_solution')

            row[f"{agent_name}_Distance"] = dist
            row[f"{agent_name}_CPU"] = 0  # Non disponible par agent en mode collaboratif
            row[f"{agent_name}_Cycles"] = agent_data.get('cycles', 0)
            row[f"{agent_name}_Deposited"] = agent_data.get('deposited', 0)

            # Gap
            if instance.optimal_value is not None:
                row[f"{agent_name}_gap"] = dist - instance.optimal_value
                row[f"{agent_name}_gap_pct"] = ((dist - instance.optimal_value) / instance.optimal_value * 100)
            else:
            ...
            if solution:
                ...
                row[f"{agent_name}_Feasible"] = feasible
            else:
                row[f"{agent_name}_Feasible"] = False
            ...
            # Agréger pour les statistiques MAS globales
            mas_total_visits += visit_stats['total_visites']

            if solution and hasattr(solution, 'visit_counter') and solution.visit_counter:
                for sig, count in solution.visit_counter.items():
                    mas_all_signatures[sig] = mas_all_signatures.get(sig, 0) + count

        # Statistiques MAS globales (tous agents combinés)
        ...
        row["MAS_Distance"] = mas_results.get('best_distance', 0)
        row["MAS_CPU"] = mas_results.get('total_time', 0)

        if instance.optimal_value is not None:
            best_dist = mas_results.get('best_distance', 0)
            row["MAS_gap"] = best_dist - instance.optimal_value
            row["MAS_gap_pct"] = ((best_dist - instance.optimal_value) / instance.optimal_value * 100)
        else:
            row["MAS_gap"] = "None"
            row["MAS_gap_pct"] = "None"

        # Faisabilité de la meilleure solution
        if mas_results.get('best_solution'):
            feasible, msg = mas_results['best_solution'].is_feasible()
            row["MAS_Feasible"] = feasible

            # Sauvegarder la meilleure solution
            mas_results['best_solution'].save_to_json("best_solution_async_mas.json")
        ...
        # Statistiques spécifiques au mode collaboratif
        row["Total_Cycles"] = mas_results.get('total_cycles', 0)
        row["Stagnation_Count"] = mas_results.get('stagnation_count', 0)
        ...
        print(f"  Cycles: {row['Total_Cycles']}, Stagnations: {row['Stagnation_Count']}")
...
def generate_plots(df, results_dir, date_str):
    ...
    results_dir = os.path.join(results_dir, f"graphics_collaborative_{date_str}")
    os.makedirs(results_dir, exist_ok=True)
    ...
    ax.set_title('Collaborative MAS - Comparaison des Distances par Agent', fontsize=14, fontweight='bold')
    ...
    if not df_with_opt.empty:
        ...
        ax.set_title('Collaborative MAS - Gaps par rapport à l\'Optimal (%)', fontsize=14, fontweight='bold')
        ax.set_xticks(x + width * len(agents) / 2)
    ...
    ax.set_title('Collaborative MAS - Comparaison des Temps de Calcul', fontsize=14, fontweight='bold')
    ax.set_xticks(x + width * len(agents) / 2)
    ax.set_xticklabels(df['Instance'], rotation=45, ha='right')
    ax.legend(loc='upper left')
    ax.grid(axis='y', alpha=0.3)
    ...
    ax.set_title('Collaborative MAS - Total Visites vs Solutions Uniques', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    # ============================================================
    # GRAPHIQUE 5: Visites cumulées par agent (stacked bar)
    # ============================================================

    fig, ax = plt.subplots(figsize=(14, 8))

    x = np.arange(len(df))
    agents_for_visits = ['HC', 'TS', 'SA', 'GA', 'ACO', 'PSO', 'GB']
    ...
    ax.set_title('Collaborative MAS - Visites Cumulées par Agent vs Total MAS', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ax.set_xticklabels(df['Instance'], rotation=45, ha='right')
    ax.legend(loc='upper left')
    ax.grid(axis='y', alpha=0.3)
    ...
    ax1.set_title('Collaborative MAS - Unique vs Redondant', fontsize=14, fontweight='bold')
    ax1.set_xticks(x)
    ...
    ax2.set_title('Collaborative MAS - Taux de Redondance', fontsize=14, fontweight='bold')
    ax2.set_xticks(x)
    ...
    if not df_with_opt.empty:
        ...
        ax.set_ylabel('Gap (%)', fontsize=12, fontweight='bold')
        ax.set_title('Collaborative MAS - Distribution des Gaps par Agent', fontsize=14, fontweight='bold')
    ...
    # ============================================================
    # GRAPHIQUE 8: Performance moyenne (radar chart)
    # ============================================================
    ...
    if not df_with_opt.empty:
        ...
        ax.set_ylim(0, 100)
        ax.set_title('Collaborative MAS - Performance Moyenne Multi-Critères', fontsize=14, fontweight='bold', pad=20)
    ...
    # ============================================================
    # GRAPHIQUE 9 (spécifique collaboratif): Cycles par instance
    # ============================================================
    ...
    if 'Total_Cycles' in df.columns:
        ...
        cycles = df['Total_Cycles'].values
        stagnations = df['Stagnation_Count'].values

        ax.bar(x - width/2, cycles, width, label='Cycles Total', color='steelblue', alpha=0.8)
        ax.bar(x + width/2, stagnations, width, label='Stagnations', color='tomato', alpha=0.8)

        ax.set_xlabel('Instances', fontsize=12, fontweight='bold')
        ax.set_ylabel('Nombre', fontsize=12, fontweight='bold')
        ax.set_title('Collaborative MAS - Cycles et Stagnations par Instance', fontsize=14, fontweight='bold')
        ax.set_xticks(x)
        ax.set_xticklabels(df['Instance'], rotation=45, ha='right')
        ax.legend()
        ax.grid(axis='y', alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(results_dir, f'cycles_stagnations_{date_str}.png'), dpi=300)
        plt.close()
        print("✓ Graphique cycles/stagnations sauvegardé")
